...
## Setup
Install dependencies with `zypper install -y make git python311-pip`.
...
- `bootstrap.sh` moved into the Makefile last year and was deleted, but the line survived. SP5 became SP6 two quarters ago. The March incident was two Marches ago.
...
If your variants map to directories, the tooling already handles it. Nested `AGENTS.md` files scope to their own subtree, and the more specific one wins on conflict — the same mental model as `.gitignore`. More precisely, at least for Codex: it concatenates every ancestor `AGENTS.md` from the project root down to the working directory, with the nearest file placed last so it overrides on conflict. A rule in the root file still applies anywhere a more specific file doesn't override it. Claude Code's CLAUDE.md hierarchy and Cursor's rules work the same way: additive, with the more specific file winning only where it actually conflicts. Most agents also take imported or path-scoped rules on top of that.
...
ifdef::sles[]
Run `zypper install -y make git python311-pip`.
endif::[]

ifdef::micro[]
Root is read-only. Run `transactional-update pkg install make git python311-pip`,
then reboot to activate the new snapshot.
Do not use plain `zypper install` on these hosts.
endif::[]
```
